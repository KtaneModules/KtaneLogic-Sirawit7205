...
public class Logic : MonoBehaviour
{
    ...
    void generateAns()
    {
        if (batt[0] > 2) keys[0] = 1;
        if (port[4] != 0) keys[1] = 1;
        if (port[1] != 0) keys[2] = 1;
        for (int i = 0; i < 6; i++)
        {
            if (serl[i] == 'A' || serl[i] == 'E' || serl[i] == 'I' || serl[i] == 'O' || serl[i] == 'U')
                keys[3] = 1;
        }
        if (keys[3] == 0) keys[4] = 1;
        if (port[5] != 0) keys[5] = 1;
        if (indc[0] != 0) keys[6] = 1;
        if (indc[6] != 0) keys[7] = 1;
        if (batt[0] == 0) keys[8] = 1;
        if (indc[5] != 0) keys[9] = 1;
        if ((serl[5] - 48) % 2 == 0) keys[11] = 1; else keys[10] = 1;
        if (indc[1] != 0) keys[12] = 1;
        if (batt[0] == 1) keys[13] = 1;
        if (batt[0] == 0) keys[14] = 1;
        if (port[3] != 0) keys[15] = 1;
        if (port[0] != 0) keys[16] = 1;
        if (batt[0] > 5) keys[17] = 1;
        if (indc[3] != 0 && indc[4] != 0) keys[18] = 1;
        if (batt[0] > 1 && port[2] != 0) keys[19] = 1;
        if (port[4] != 0 && port[1] != 0) keys[20] = 1;
        if (indc[2] != 0) keys[21] = 1;
        if (serl[0] < 65 && serl[1] < 65 && serl[2] < 65 && serl[3] < 65 && serl[4] < 65 && serl[5] < 65) keys[22] = 1;
        if (port[6] > 3) keys[23] = 1;
        if (indc[7] == 0) keys[24] = 1;
        if (port[4] != 0 && port[3] != 0) keys[25] = 1;
